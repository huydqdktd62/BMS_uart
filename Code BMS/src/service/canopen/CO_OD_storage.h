...
static inline void CO_OD_STORAGE_update_crc16_comm_from_data_in_ram(CO_OD_Storage* this)
{
    uint16_t comm_para_crc16_val = CO_CRC_CalculateCRC16((uint8_t*)this->comm_para.p_data, this->comm_para.size);
    CO_OD_SIGN_set_crc16_comm_para(&this->sign, comm_para_crc16_val);
}

static inline void CO_OD_STORAGE_update_crc16_app_from_data_in_ram(CO_OD_Storage* this)
{
	uint16_t app_para_crc16_val = CO_CRC_CalculateCRC16(
			(uint8_t*) (this->app_para.p_data + this->image.boot_setting_size),
			this->app_para.size - this->image.boot_setting_size);
    CO_OD_SIGN_set_crc16_app_para(&this->sign, app_para_crc16_val);
}
