...
void multiply(Matrix A, Matrix B, Matrix result) {
	...
	R.row = result.row;
	R.col = result.col;
	R.entries = malloc(sizeof(int64_t) * result.row * result.col);
	uint8_t i, j, k;
	for (i = 1; i <= A.row; i++)
		for (j = 1; j <= B.col; j++) {
			int64_t de = 0;
...
void chol(Matrix src_matrix, Matrix dst_matrix) {
	...
	mediate.entries = (int64_t*) malloc(
			sizeof(int64_t) * mediate.row * mediate.col);
	...
	for (i = 1; i <= mediate.row; i++) {
		...
		mediate.entries[(i - 1) * mediate.col + i - 1] = square_root_int64(
				absolute_int64(de));
	}
